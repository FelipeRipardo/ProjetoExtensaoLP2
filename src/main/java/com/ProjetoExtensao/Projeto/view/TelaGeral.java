...
import com.ProjetoExtensao.Projeto.repositorios.ConsultaRepositorio; // NOVO
import com.ProjetoExtensao.Projeto.repositorios.PacienteRepositorio; // NOVO
...
public class TelaGeral extends JFrame {
    ...
    // --- INJEÇÃO DOS NOVOS REPOSITÓRIOS ---
    @Autowired
    ...
    private JPanel createMainPanel() {
        ...
        //Exibe dados reais agora que temos os repositórios, ou mantém fixo se preferir
        statsPanel.add(createStatItem("Pacientes", String.valueOf(pacienteRepositorio.count()), Cores.COR_VERMELHO_IDOSAS));
        statsPanel.add(createStatItem("Consultas", String.valueOf(consultaRepositorio.count()), Cores.COR_VERDE_ENFERMARIA));
        //statsPanel.add(createStatItem("Visitas", "2", Cores.COR_VERDE_ENFERMARIA)); // Mantendo fixo por enquanto
        ...
        // --- NOVO: Botão Relatórios (Ativado) ---
        JButton btnRelatorios = createDashboardButton("Relatórios", "relatorios.png");
        btnRelatorios.addActionListener(e -> abrirTelaRelatoriosPopup());
        ...
        gridPanel.add(btnRelatorios); // Adiciona o botão ativado
    ...
    //Método auxiliar para abrir a nova tela como um Pop-up
    private void abrirTelaRelatoriosPopup() {
        JDialog dialog = new JDialog(this, "Relatórios e Indicadores", true); // true = Modal (bloqueia fundo)
        dialog.setSize(800, 600);
        dialog.setLocationRelativeTo(null);

        //Cria a tela passando os repositórios injetados
        TelaRelatorios telaRelatorios = new TelaRelatorios(pacienteRepositorio, consultaRepositorio);
        dialog.add(telaRelatorios);
